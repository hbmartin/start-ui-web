...
const finalize = (
  ...
  const dbTimings = timingStore.getStore()?.prisma ?? [];
...
const handleError = (error: unknown, procedureLogger: ProcedureLogger) => {
  ...
  return mapPrismaError(error, procedureLogger);
};
...
function mapPrismaError(
    ...
    return match(error.code)
      .with('P2002', () => {
        ...
        return new ServerFnError('CONFLICT', {
          ...
          data: { target: error.meta?.target as string[] | string | undefined },
        });
      })
      .with('P2025', () => {
        procedureLogger.warn(
        ...
        return new ServerFnError('NOT_FOUND', {
  ...
  if (error instanceof Prisma.PrismaClientValidationError) {
    procedureLogger.error(`Prisma Client Validation Error: ${error.message}`);
    return new ServerFnError('BAD_REQUEST', {
  ...
  return error instanceof Error
    ? error
    : new ServerFnError('INTERNAL_SERVER_ERROR', {
...
export async function withPublicContext<T>(
  ...
  return await timingStore.run({ prisma: [] }, async () => {
    try {
